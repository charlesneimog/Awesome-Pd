...
- :octicons-download-16: __Download__ via [Deken](../deken.md).  <p style="font-size: 14px">_Open `Pd` and go to `Tools`:material-arrow-right:`Find Externals`. Search for <code>timbreIDLib</code> and install it. Then create an object with `declare -lib timbreIDLib -path timbreIDLib`. Finally, use `tabletool` or any other object from `timbreIDLib`._</p>
