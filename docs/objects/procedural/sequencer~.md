...
The `sequencer~` object outputs values from a predefined sequence when triggered by a signal (0 to non-0 transition) or a bang. You can set the sequence using a list of floats and navigate to specific positions within it. It also outputs an impulse when the end of the sequence is reached.
