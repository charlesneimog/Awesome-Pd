...
- :octicons-download-16: __Download__ via [Deken](../deken.md).  <p style="font-size: 14px">_Open `Pd` and go to `Tools`:material-arrow-right:`Find Externals`. Search for <code>fftease</code> and install it. Then create an object with `declare -lib fftease -path fftease`. Finally, use `resent~` or any other object from `fftease`._</p>
