...
The `minimum~` object outputs a signal that represents the minimum value between two input signals, or between an input signal and a numerical argument. This object is considered redundant and deprecated; users are advised to use Pure Data Vanilla's `min~` object instead for similar functionality.
